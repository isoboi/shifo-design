  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="mb-4 md:mb-6">
        ...
        <p className="text-sm md:text-base text-gray-600 mt-1 md:mt-2">Добро пожаловать в систему управления клиникой</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs md:text-sm font-medium text-gray-600">Записи сегодня</p>
              <p className="text-xl md:text-2xl font-bold text-gray-900">{todayAppointments.length}</p>
            </div>
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs md:text-sm font-medium text-gray-600">Доход за месяц</p>
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs md:text-sm font-medium text-gray-600">Ожидают оплаты</p>
              <p className="text-xl md:text-2xl font-bold text-gray-900">{pendingPayments.length}</p>
            </div>
            <div className="h-10 w-10 md:h-12 md:w-12 bg-rose-100 rounded-lg flex items-center justify-center">
              <AlertTriangle className="h-5 w-5 md:h-6 md:w-6 text-rose-600" />
            </div>
          </div>
          <div className="mt-3 md:mt-4 flex items-center text-xs md:text-sm">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <div className="p-4 md:p-6 border-b border-gray-100">
          <h2 className="text-lg md:text-xl font-semibold text-gray-900">Ближайшие записи</h2>
        </div>
        <div className="p-4 md:p-6">
          {upcomingAppointments.length === 0 ? (
            <p className="text-sm md:text-base text-gray-500 text-center py-6 md:py-8">Нет запланированных записей</p>
          ) : (
            <div className="space-y-3 md:space-y-4">
              {upcomingAppointments.map((appointment) => {
                ...
                return (
                  <div
                    key={appointment.id}
                    className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 md:p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors gap-3 sm:gap-4"
                  >
                    <div className="flex items-center space-x-3 md:space-x-4">
                      <div className="h-10 w-10 bg-gradient-to-r from-sky-400 to-blue-500 rounded-full flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-3 sm:gap-4">
                      <div className="text-left sm:text-right">
                        ...
                        <p className="text-xs md:text-sm text-gray-500">{appointment.time}</p>
                      </div>
